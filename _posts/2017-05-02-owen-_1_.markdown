---
layout: post
title:  "owenzhang _1_ 代码解析!"
date:   2017-05-02 09:18:47 +0800
...
raw_data_path = utils.raw_data_path  # 源数据路径
tmp_data_path = utils.tmp_data_path  # 临时文件路径


t0org0 = pd.read_csv(open(raw_data_path + "train", "ra"))  # 训练数据集
h0org = pd.read_csv(open(raw_data_path + "test", "ra"))  # 测试数据集
...
#day 当天
#hour1 当天小时
#day_hour 当前距离21号的小时数
#day_hour_prev 当前距离21号的小时数前一个小时
#day_hour_next 当前距离21号的小时数后一个小时
#app_or_web app或者web
#app_id=='ecad2386'的特征app_or_web赋值1
...
t0 = t0org
#添加特征app_site_id 值为app_id和site_id相连
...
# 添加对应的exptv扩展字段,实质上在寻找哪种组合类别对应click的加总和数量情况,如exptv_app_or_web
...
# 前一天
t0['pday'] = t0.day - 1
# 寻找device_ip和pday、day对应click的加总和数量情况
...
# app_or_web=0,hour1_web = -1
t0.ix[t0.app_or_web.values==0, 'hour1_web'] = -1
# 统计app_id和device_ip的匹配出现的次数
...
# 统计device_ip的相同出现次数
# 统计device_ip&day的相同出现次数
# 统计app_id&device_ip的相同出现次数
...
#复制cnt_dev_id后与其比较，若大于300，替换该值等于300
#复制cnt_dev_ip后与其比较，若大于300，替换该值等于300
...
#复制device_id，将其中等于1的值替换为___only1
#复制device_ip，将其中等于1的值替换为___only1
